...
class Interval {
public:
  Interval(int l, int h, int k):low(l), high(h), key(k){};
  ~Interval();
  int GetLowPoint(){return low;};
  int GetHighPoint(){return high;};
  void Print(){printf("%d--%d (%d)", low, high, key);};
  int low, high, key;
};
...
public:
  ...
  void Enumerate(int low, 
              int high, vector<int> &result) ;
